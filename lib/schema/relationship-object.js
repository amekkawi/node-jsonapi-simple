...
util.superClass(RelationshipObject, BaseMembersObject.prototype);
			...
			if (util.isArray(value)) {
				for (var i = 0, l = value.length; i < l; i++) {
					if (!(value[i] instanceof ResourceIdentifierObject))
						throw new InvalidMemberValueError(util.getProtoName(this), 'must be an object (ResourceIdentifierObject)', i, [key]);
				...
				if (!(value instanceof ResourceIdentifierObject))
					throw new InvalidMemberValueError(util.getProtoName(this), 'must be an object (ResourceIdentifierObject)', key);
